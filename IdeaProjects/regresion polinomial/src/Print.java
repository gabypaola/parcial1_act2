...
public class Print {
    ...
    Prediciones Prediciones  = new Prediciones();
    ...
    //double predi = Prediciones.predecir(data.C,coeficientes);
   ...
   public void printmatrizTranspuesta(){
    ...
    public void printPredicciones(double[] C, double[] coeficientes) {
        Prediciones prediccionesObj = new Prediciones();
        double[] predicciones = prediccionesObj.calcularPredicciones(C, coeficientes);


        System.out.println("----------- Predicciones: --------------");
        for (int i = 0; i < C.length; i++) {
            System.out.printf("Para x = %.2f, la predicción es y = %.2f%n", C[i], predicciones[i]);
        }
    }
